...
(function () {
  ...
  const DIVISION_LABEL = { MANAJEMEN: "Manajemen", DAPUR: "Dapur", LAYANAN: "Layanan" };
  ...
  /* ---------------- Division colors (track live theme) ---------------- */
  function hexToRgb(hex) {
    hex = String(hex).trim().replace("#", "");
    if (hex.length === 3) hex = hex.split("").map((c) => c + c).join("");
    const n = parseInt(hex, 16);
    if (isNaN(n)) return "122, 63, 208";
    return ((n >> 16) & 255) + ", " + ((n >> 8) & 255) + ", " + (n & 255);
  }
  function divisionColor(d) {
    if (d === "MANAJEMEN") return cssVar("--accent", "#7A5DC7");
    if (d === "DAPUR") return cssVar("--pink", "#E89AAE");
    return cssVar("--caramel", "#DDA45E"); // LAYANAN
  }
  // Darken a hex toward black (for readable name text on the light dropdown).
  function darkenHex(hex, f) {
    const p = hexToRgb(hex).split(",").map((n) => Math.round(Math.max(0, Math.min(255, parseInt(n, 10) * f))));
    return "#" + p.map((n) => n.toString(16).padStart(2, "0")).join("");
  }
  function nameColor(d) { return darkenHex(divisionColor(d), 0.62); }
  ...
  /* ---------------- State + API layer ---------------- */
  let state = { staff: [], week: { key: "", start: "", end: "", records: [] }, history: [], payroll: { jab: {} }, theme: { accent: "#7A5DC7", pink: "#E89AAE", bg: "vanilla" }, passwordIsDefault: true };
  let authed = false;
  let TOKEN = null;
  let staffSig = "";
  let recapRange = "week"; // "week" | "7" | "14" | "30"

  // Mount prefix this app is served under (e.g. "/coolbeans"), so API calls
  // hit /coolbeans/api/... when the combined server hosts both webs.
  ...
  function afterStateUpdate() {
    ...
    if (state.theme && !themeBusy) {
      const tsig = JSON.stringify(state.theme);
      if (tsig !== appliedThemeSig) { appliedThemeSig = tsig; applyTheme(state.theme); }
    }
  ...
  function fillNameSelect() {
    ...
    DIVISIONS.forEach((div) => {
      ...
      html += `<optgroup label="${esc(DIVISION_LABEL[div] || div)}" style="color:${divisionColor(div)}">`;
      const oc = nameColor(div);
      list.sort((a, b) => a.name.localeCompare(b.name)).forEach((s) => {
        html += `<option value="${esc(s.id)}" style="color:${oc}">${esc(s.name)}</option>`;
      });
      html += "</optgroup>";
    });
    sel.innerHTML = html;
  ...
  function updateTotalPreview() {
    ...
    qs("#tpHuman").textContent = sec ? "Setara " + secToHuman(sec) + " jam kerja" : "Isi jam Masuk & Keluar untuk menghitung";
  }
  ...
  async function submitAbsen(e) {
    ...
    if (!staff) return toast("Pilih nama karyawan dulu", "err");
    if (!date) return toast("Pilih tanggal", "err");
    if (!on || !off) return toast("Isi jam Masuk & Pulang", "err");
  ...
  function renderRecent() {
    ...
    list.innerHTML = recs.map((r) => `
      <li class="recent-item">
        ...
        <span class="ri-main">
          <span class="ri-name" style="color:${nameColor(r.division || divisionOf(r.jabatan))}">${esc(r.name)}</span>
  ...
  function renderHeroMeta() {
    ...
    qs("#heroStaff").textContent = activeStaff().length + " karyawan aktif";
  }
  ...
  function renderKPI() {
    ...
    const cards = [
      { ic: '<path d="M12 7v5l3 2" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round"/><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="1.8" fill="none"/>', label: "Total Jam Kerja", value: hours1(tSec), unit: "jam", foot: secToHuman(tSec) + " · " + rangeLabel(recapRange) },
      { ic: '<circle cx="9" cy="8" r="3.2" stroke="currentColor" stroke-width="1.8" fill="none"/><path d="M3.5 19a5.5 5.5 0 0 1 11 0M16 8.5a3 3 0 0 1 0 5M17 19a5 5 0 0 0-2-4" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round"/>', label: "Karyawan Aktif", value: active, unit: "", foot: "dari " + activeStaff().length + " terdaftar" },
      ...
      { ic: '<path d="M4 19V5M4 19h16M8 16l3-4 3 2 4-6" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round"/>', label: "Rata-rata / Karyawan", value: hours1(avg), unit: "jam", foot: "per karyawan aktif" },
    ];
  ...
  function renderChartDivision() {
    ...
    const segs = DIVISIONS.map((d) => {
      ...
      const seg = `<circle cx="70" cy="70" r="${R}" fill="none" stroke="${divisionColor(d)}" stroke-width="20"
    ...
    const ink = cssVar("--ink", "#2F1B4B"), muted = cssVar("--muted", "#9588A8");
    const ring = total ? segs : `<circle cx="70" cy="70" r="${R}" fill="none" stroke="rgba(${hexToRgb(divisionColor("MANAJEMEN"))},.12)" stroke-width="20"/>`;
    qs("#chartDivision").innerHTML = `
      <svg class="donut" width="150" height="150" viewBox="0 0 140 140" aria-hidden="true">
        ${ring}
        <text x="70" y="64" text-anchor="middle" class="donut-center" fill="${ink}" font-size="22" font-weight="600">${hours1(total)}</text>
        <text x="70" y="84" text-anchor="middle" fill="${muted}" font-size="10" letter-spacing="2">JAM</text>
      </svg>
      <div class="legend">
        ${DIVISIONS.map((d) => `
          <div class="legend-item">
            <span class="legend-dot" style="background:${divisionColor(d)}"></span>
            <span class="legend-name">${esc(DIVISION_LABEL[d] || d)}</span>
  ...
  function renderChartRank() {
    const desc = qs("#rankDesc"); if (desc) desc.textContent = "Total jam kerja tertinggi · " + rangeLabel(recapRange);
  ...
  function divisionPill(division) {
    const c = divisionColor(division);
    const rgb = hexToRgb(c);
    return `<span class="pill" style="color:${c};border-color:rgba(${rgb},.45);background:rgba(${rgb},.10)">${esc(DIVISION_LABEL[division] || division)}</span>`;
  }
  ...
  function renderRecapTable() {
    ...
    body.innerHTML = agg.map((o) => `
      <tr class="clickable" data-name="${esc(o.name)}" data-jab="${esc(o.jabatan)}">
        <td class="td-name" style="color:${nameColor(o.division)}">${esc(o.name)}</td>
        <td>${divisionPill(o.division)} <span style="color:var(--ink-2)">${esc(o.jabatan)}</span></td>
    ...
    showModal(`
      ...
      <div class="table-scroll">
        <table class="ledger compact">
          <thead><tr><th>Tgl</th><th>Shift</th><th>Masuk – Keluar</th><th class="num">Durasi</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>`);
  }
  ...
  function renderArchive() {
    ...
    rail.innerHTML = items.map((w, i) => {
      ...
      return `<button class="coin ${i === 0 ? "newest" : ""}" data-key="${esc(w.key)}">
          ...
          <div class="coin-stat"><span>Karyawan</span><b>${people}</b></div>
  ...
  function openArchiveWeek(key) {
    ...
    const payrollBlock = agg.length ? `
        ...
        <table class="ledger compact">
          <thead><tr><th>Karyawan</th><th>Jabatan</th><th class="num">Jam</th><th class="num">Lembur</th><th class="num">Gaji/Jam</th><th class="num">Gaji</th></tr></thead>
          <tbody>${payRows}</tbody>
        </table>
    ...
    showModal(`
      ...
      <div class="detail-kpis">
        ...
        <div class="detail-kpi"><b>${new Set(w.records.map((r) => r.name)).size}</b><span>Karyawan</span></div>
      ...
      <div class="table-scroll">
        ...
        <table class="ledger compact">
          <thead><tr><th>Karyawan</th><th>Jabatan</th><th class="num">Hari</th><th class="num">Shift</th><th class="num">Jam</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
        ${payrollBlock}
      </div>
  ...
  function renderStaffTable() {
    ...
    body.innerHTML = sorted.map((s) => {
      const opts = DIVISIONS.map((d) => `<optgroup label="${esc(DIVISION_LABEL[d] || d)}" style="color:${divisionColor(d)}">${JABATAN[d].map((j) => `<option value="${esc(j)}"${j === s.jabatan ? " selected" : ""}>${esc(j)}</option>`).join("")}</optgroup>`).join("");
      return `<tr>
        ...
        <td><div class="row-tools">
          <button class="icon-btn" data-del="${esc(s.id)}" title="Hapus karyawan" aria-label="Hapus ${esc(s.name)}"><svg viewBox="0 0 24 24" width="15" height="15"><path d="M5 7h14M10 7V5h4v2M6 7l1 12h10l1-12" stroke="currentColor" stroke-width="1.7" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg></button>
        </div></td>
      </tr>`;
    }).join("");
    ...
    qsa("[data-del]", body).forEach((b) => b.addEventListener("click", async () => {
      ...
      if (await confirmDialog(`Hapus <b>${esc(s.name)}</b> dari daftar karyawan?`, "Catatan kehadiran yang sudah masuk tetap tersimpan.")) {
        try { await api("/api/staff/" + s.id, { method: "DELETE" }); await refresh(); toast("Karyawan dihapus", "info"); }
        catch (err) { toast(err.message, "err"); }
      }
    }));
  }
  ...
  async function addStaff(e) {
    ...
    if (!name) return toast("Isi nama karyawan", "err");
    try {
  ...
  function renderPayroll() {
    ...
    qs("#payrollHint").textContent = !anyRate
      ? "Gaji belum diatur. Manajemen dapat mengaturnya di Pengaturan Gaji."
  ...
  function renderPayrollSettings() {
    ...
    if (host.contains(document.activeElement)) return; // jangan ganggu input manajemen
    const p = state.payroll || { jab: {} };
    const rows = DIVISIONS.map((d) => {
      const head = `<tr class="rate-div"><td colspan="4">${esc(DIVISION_LABEL[d] || d)}</td></tr>`;
  ...
  /* ===================================================================
     ACCESS ROLE (pengamat vs manajemen)
     =================================================================== */
  function applyRole() {
    ...
    if (b) { b.textContent = authed ? "Mode Manajemen" : "Mode Pengamat"; b.classList.toggle("admin", authed); }
  }
  function viewOnly() {
  ...
  function exportCSV(records, fname) {
    const head = ["Tanggal", "Hari", "Nama", "Jabatan", "Divisi", "Shift", "Jam Masuk", "Jam Keluar", "Durasi (jam)", "Durasi"];
  ...
  async function gotoDash() {
    ...
    // Pengamat boleh langsung lihat dashboard tanpa login
  ...
  async function tryLogin(e) {
    ...
    try {
      ...
      await refresh(); renderDashboard(); showView("dash"); toast("Selamat datang, Manajemen", "ok");
  ...
  /* ===================================================================
     COLOR CUSTOMIZER — tema global (disetel manajemen, disimpan di server)
     =================================================================== */
  ...
  const PRESETS = [
    ...
    { name: "Caramel", accent: "#A6713F", pink: "#E0A86A" },
    { name: "Blueberry", accent: "#5A6BD6", pink: "#9C8BEA" },
  ];
  const BG_PRESETS = {
    vanilla: { id: "vanilla", bg: "#FBF1E6", bg2: "#F3E3D2", surface: "#FFFBF7", surface2: "#FBF3EA", inset: "#F6EADB", swatch: "#FBF1E6" },
    strawberry: { id: "strawberry", bg: "#FCEEF0", bg2: "#F8DCE2", surface: "#FFFBFC", surface2: "#FDF1F4", inset: "#FAE6EB", swatch: "#F8DCE2" },
    matcha: { id: "matcha", bg: "#F1F2E4", bg2: "#E4E8CE", surface: "#FCFCF6", surface2: "#F4F5E8", inset: "#ECEEDB", swatch: "#E4E8CE" },
    taro: { id: "taro", bg: "#F3EEFB", bg2: "#E6DCF6", surface: "#FFFCFF", surface2: "#F6F1FD", inset: "#EDE6F9", swatch: "#E6DCF6" },
    choco: { id: "choco", bg: "#F2E9DF", bg2: "#E6D6C4", surface: "#FDFAF6", surface2: "#F5ECE2", inset: "#EFE2D3", swatch: "#E6D6C4" },
  };
  ...
  function setThemeVars(t) {
    ...
    r.setProperty("--pink-rgb", hexToRgb(t.pink));
    const bg = BG_PRESETS[t.bg] || BG_PRESETS.vanilla;
    r.setProperty("--bg", bg.bg); r.setProperty("--bg-2", bg.bg2);
    r.setProperty("--surface", bg.surface); r.setProperty("--surface-2", bg.surface2);
    r.setProperty("--inset", bg.inset);
  }
  function applyTheme(t) { currentTheme = Object.assign({}, DEFAULT_THEME, t); setThemeVars(currentTheme); syncThemeControls(); }
  function syncThemeControls() {
    const a = qs("#cAccent"), p = qs("#cPink");
    if (a) a.value = currentTheme.accent; if (p) p.value = currentTheme.pink;
    qsa(".preset-chip").forEach((c) => c.classList.toggle("active", c.dataset.accent.toLowerCase() === currentTheme.accent.toLowerCase() && c.dataset.pink.toLowerCase() === currentTheme.pink.toLowerCase()));
    qsa(".bg-swatch").forEach((s) => s.classList.toggle("active", s.dataset.bg === currentTheme.bg));
  }
  async function saveThemeServer() {
    appliedThemeSig = JSON.stringify(currentTheme);
    try { await api("/api/theme", { method: "POST", body: currentTheme }); }
    catch (err) { toast("Gagal menyimpan tema: " + err.message, "err"); }
  }
  function refreshThemeColors() {
    // Most of the UI recolors instantly via CSS vars; only re-render the
    // JS-painted bits (donut strokes, division pills) when dashboard is open.
    if (!qs("#view-dashboard").classList.contains("hidden")) {
      renderChartDivision(); renderRecapTable(); renderPayroll(); renderStaffTable();
    }
  }
  function buildThemeControls() {
    const pg = qs("#presetGrid");
    pg.innerHTML = PRESETS.map((p) => `<button class="preset-chip" type="button" title="${esc(p.name)}" aria-label="${esc(p.name)}" data-accent="${p.accent}" data-pink="${p.pink}" style="background:linear-gradient(135deg,${p.accent},${p.pink})"></button>`).join("");
    qsa(".preset-chip", pg).forEach((c) => c.addEventListener("click", () => {
      applyTheme(Object.assign({}, currentTheme, { accent: c.dataset.accent, pink: c.dataset.pink }));
      saveThemeServer(); refreshThemeColors();
    }));
    const bgGrid = qs("#bgGrid");
    bgGrid.innerHTML = Object.keys(BG_PRESETS).map((k) => { const b = BG_PRESETS[k]; return `<button class="bg-swatch" type="button" title="${esc(b.id)}" aria-label="Latar ${esc(b.id)}" data-bg="${b.id}" style="background:${b.swatch}"></button>`; }).join("");
    qsa(".bg-swatch", bgGrid).forEach((s) => s.addEventListener("click", () => {
      applyTheme(Object.assign({}, currentTheme, { bg: s.dataset.bg }));
      saveThemeServer(); refreshThemeColors();
    }));
  }
  function initThemeUI() {
    buildThemeControls();
    setThemeVars(currentTheme); syncThemeControls(); // tema awal (default) sampai server memuat

    const live = (which, val) => {
      themeBusy = true;
      currentTheme[which] = val;
      const r = document.documentElement.style;
      r.setProperty(which === "accent" ? "--accent" : "--pink", val);
      r.setProperty(which === "accent" ? "--accent-rgb" : "--pink-rgb", hexToRgb(val));
    };
    const commit = () => { themeBusy = false; syncThemeControls(); saveThemeServer(); refreshThemeColors(); };
    qs("#cAccent").addEventListener("input", (e) => live("accent", e.target.value));
    qs("#cPink").addEventListener("input", (e) => live("pink", e.target.value));
    qs("#cAccent").addEventListener("change", commit);
    qs("#cPink").addEventListener("change", commit);

    qs("#themeReset").addEventListener("click", () => {
      applyTheme(DEFAULT_THEME); saveThemeServer(); refreshThemeColors();
      toast("Warna dikembalikan ke default", "info");
    });
  }
  ...
  async function init() {
    // theme first so there is no color flash
    initThemeUI();

    // static form setup (independent of server data)
    setJabatanDisplay("");
    fillJabatanSelect("#saJabatan", "TRAINER");
    ...
    qs("#btnResetAll").addEventListener("click", async () => {
      if (await confirmDialog("Reset SEMUA data?", "Karyawan, rekap, dan arsip dikembalikan ke awal (kata sandi tetap). Tidak bisa dibatalkan.")) {
