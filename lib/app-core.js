/* =====================================================================
   ...
   two instances (Cool Beans + RS Senjakala) on different path prefixes.
   ===================================================================== */
"use strict";
const fs = require("fs");
  ...
  function load() {
    try {
      if (fs.existsSync(DATA_FILE)) {
        ...
        state = JSON.parse(fs.readFileSync(config.seedFile, "utf8"));
        persist();
        console.log(`[${config.name}] Seeded data from ${path.basename(config.seedFile)}.`);
      } else {
        state = defaultState(); persist();
        console.log(`[${config.name}] Created new data file with seeded staff.`);
      }
    } catch (e) {
      console.error(`[${config.name}] Failed to read data, starting fresh:`, e.message);
      state = defaultState(); persist();
    }
  ...
  function persist() {
    ...
    writeTimer = setTimeout(() => {
      try {
        ...
        const tmp = DATA_FILE + ".tmp"; fs.writeFileSync(tmp, JSON.stringify(state)); fs.renameSync(tmp, DATA_FILE);
      } catch (e) { console.error(`[${config.name}] Persist failed:`, e.message); }
    }, 80);
  }
